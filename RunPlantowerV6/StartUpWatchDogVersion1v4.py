...
#Change as needed:
RunPMShell = '/home/particulatepi/RunPlantower/RunPMShell.sh'
...
def doWatchDog(print2screen):
    ...
    while(ScriptsUp == 0):
        ...
        PythonUp = subprocess.check_output(['grep', 'python3.9'], stdin=PythonUpTEMP.stdout)
        PythonUp = PythonUp.decode("utf-8")

        serverStart = PythonUp.find("UpdateServer")
        plantowerStart = PythonUp.find("PMS5003stVersion")
        serialConnectionStart = PythonUp.find("ttyAMA0")
        usbConnectionStart = PythonUp.find("ttyAMA1")
        ...
        if((serverStart == -1)|(plantowerStart == -1)|(serialConnectionStart == -1)|(usbConnectionStart == -1)|(zeroStart == -1)):
    ...
    if (print2screen == 1):
        print('SensOR clock says the time is:')
        print(datetime.datetime.now())
        print('Check if the datafile contains the current time:')
    ...
    #(5)About SensOR
    if (print2screen == 1):
        print('About this sensor:')
        print(subprocess.run(['lsb_release', '-irdc']))
        print(subprocess.run(['cat', '/proc/device-tree/model']))
